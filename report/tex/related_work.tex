...
In this section we will explore three topics concerning this project.
First of all, we will dive into spectral graph theory.
Then, we will deal with 2D image processing, focusing three applications: denoising, sharpening and image segmentation.
For each of them, we show a classical algorithm, a new spectral approach and then a quick comparison of them.
Finally, we present what has been done in 3D image processing so far using spectral graph theory, essentially using the graph laplacian operator.

\subsection{Spectral graph theory}

Spectral graph theory has a long history starting with matrix theory and linear algebra that were used to analyse adjancency matrices of graphs.
It consists in studying the properties of graphs in relation to the eigenvalues and eigenvectors of the adjancency or Laplacian matrix of the graph.
The eigenvalues of such a matrix are called the spectrum of the graph.
The second-smallest eigenvalue has been called ``algebraic connectivity of a graph" by Fiedler \cite{fiedler_algebraic_1973}, and is therefore also known as \textit{Fiedler value}.

This field is very broad and the eigen-decomposition of graphs is used in a lot of areas.
First, it was applied to chemistry because eigenvalues can be associated with the stability of molecules.
Spectral graph theory has a many other applications such as graph coloring, graph isomorphism testing, random walks and graph partitioning among others.
...
Since the adjancency matrix of a graph only holds basic information about it, we usually augment it to the Laplacian matrix.
Multiple definitions of the Laplacian matrix are given in \cite{chung_spectral_1997} and each hold different properties.
...
The Cheeger constant measures the degree of ``bottleneck" of a graph, useful for constructing well-connected graphs.
Considering a graph \(G\) of \(n\) vertices, the Cheeger constant is defined as
...
\[\partial S = {{u, v} \in E(G) : u \in S and v \notin S},\]
...
\[\lambda_1(\Lapl) \ge \frac{h^2(\Lapl)}{4}.\] % TODO cite Cheeger
