...
\(K_B\) holding the similiarities between the sampled pixels and the remaining pixels being of size \(p \times (N-p)\).
...
% TODO c'est correct tout ca???? parce qu'exposé dit f(Pi) et GLIDE dit Pi^k ?????????
In the case of a function that applies a power \(k\) to the Laplacian \(\Lapl\), we can say that \(\Lapl^k = \Phi \Pi^k \Phi^T\).
