...
We averaged the runs to avoid noise.
...
We consider a picture, of size \(721 \times 558\), 402 318 pixels, and we sample 1\%, meaning 4032 sample pixels.
...
To make this clear, we present below the porportion of the inverse iteration in the whole algorithm:
...
This confirms that the algorithm is not totally scalable.
Finally, we look at the internal steps in the inverse power method to see what prevents the scalability.
...
We observe clearly that the Gram-Schmidt orthogonalisation is the limiting factor and takes becomes the most time-consuming step of the inverse iteration.
